...
evaluates validation BPB at seq_len = 512, 1024, 2048.  This demonstrates
YaRN's context-extension benefit: the baseline degrades sharply beyond its
training seq_len=512, while YaRN (alpha=4.0) stays flatter.

Usage (run from repo root after downloading checkpoints from Modal volume):
    python assignments/eval_extended_context.py

Optional arguments (set via environment or edit the CONFIG block below):
...
def main():
    ...
    for seq_len in SEQ_LENS:
        ...
        for tag, model in models.items():
            # YaRN model at seq_len > train_seq_len: rotary cache already extended above
